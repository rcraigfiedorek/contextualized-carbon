...
from api.facts.quantity import ureg
...
tcre_fact = FactTemplate(
    conversion=TRANSIENT_CLIMATE_RESPONSE_TO_EMISSIONS,
    message='This alone will raise the mean global temperature by %s.'
)
...
empire_state_fact = FactTemplate(
    conversion=EMPIRE_STATE_BUILDINGS_PER_CO2,
    message='This amount of carbon dioxide has the same mass as %s Empire State Buildings.'
)
...
taylor_jet_fact = FactTemplate(
    ...
    message=(
        ...
        'equator once every %s for an entire year.'
    ),
...
arctic_sea_ice_fact = FactTemplate(
    conversion=ARCTIC_SEA_ICE_MELT_PER_EMISSIONS,
    message='This amount of carbon alone will cause the melting of %s of ice in the Arctic.'
)
ALL_FACTS.append(arctic_sea_ice_fact)

TROPICAL_FOREST_SEQURESTRATION = 11 * ureg.t * ureg.co2 / ureg.yr / ureg.ha
TROPICAL_FOREST_AREA_TIME_PER_EMISSION = 1 / TROPICAL_FOREST_SEQURESTRATION
tropical_forest_fact = FactTemplate(
    conversion=TROPICAL_FOREST_AREA_TIME_PER_EMISSION,
    message='It would take %s of tropical forest to absorb this amount of carbon per year from the atmosphere.'
)
ALL_FACTS.append(tropical_forest_fact)

NEW_ENGLAND_FOREST_SEQURESTRATION = 0.5 * ureg.t * ureg.co2 / ureg.yr / ureg.ha
NEW_ENGLAND_FOREST_AREA_TIME_PER_EMISSION = 1 / TROPICAL_FOREST_SEQURESTRATION
new_england_forest_fact = FactTemplate(
    conversion=NEW_ENGLAND_FOREST_AREA_TIME_PER_EMISSION,
    message='It would take %s of New England forests to absorb this amount of carbon per year from the atmosphere.'
)
...
poisoned_area_fact = FactTemplate(
    conversion=AIR_COLUMN_POISONED_PER_CO2,
    message=(
        'Releasing this amount of carbon dioxide into %s of Earth\'s air '
        'column would cause lethal carbon dioxide poisoning for all humans '
        'in this area.'
    )
)
...
ccus_fact = FactTemplate(
    ...
    message=(
        ...
        'Yet it would take %s for the entire planet\'s CCUS operations to capture this amount of carbon.'
    )
)
...
dac_fact = FactTemplate(
    conversion=DAC_TIME_PER_CO2,
    message=(
        'Direct Air Capture (DAC) is often touted as a solution to carbon emissions reduction. '
        'Yet it would take %s for the entire planet\'s DAC operations to capture this amount of carbon.'
    )
)
...
carpet_fact = FactTemplate(
    conversion=AREA_OF_CM_CARPET_PER_CO2,
    message=(
        'This is enough carbon dioxide to cover %s of land '
