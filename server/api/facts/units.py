...
class PrettyUnit:
    ...
    threshold: float | None = (
        None  # The fallback unit will be used as the threshold if none is given
    )
    ...
    def pformat(self, q: Quantity, _recursive_depth: int = 0) -> str:
        ...
        if self.fallback_punit is not None:
            if self.threshold is not None:
                if m >= self.threshold:
                    return self.fallback_punit.pformat(
                        q, _recursive_depth=_recursive_depth
                    )
            else:
        ...
        if 0.1 <= m < 1_000:
            ...
            formatted_number = PrettyUnit.unitless.pformat(
                m * ureg.one, _recursive_depth=_recursive_depth + 1
            )
...
PFORMAT_SUPPORTED_DIMENSIONS: Dict[pint.util.UnitsContainer, PrettyUnit] = {
    ureg.UnitsContainer({}): PrettyUnit.unitless,
    ureg.UnitsContainer({"[temperature]": 1}): PrettyUnit(
        ureg.delta_degC, "degree Celsius", "degrees Celsius"
    ),
